...
<CollapsableSection name="Property" bind:collapsed={$collapsed} class={"timeline-property-setting"}>
	<TimelineNoteSorterPropertySelect
		tabindex={0}
		order={selector.timelineNoteSorterSelector}
		onSelected={() => dispatch("propertySelected", selector.selectedProperty())}
	/>

	<NumericPropertyIntToggle property={selector.selectedProperty()} tabindex={1} />
</CollapsableSection>
